...
# Dashboard title
st.title("🌟 ManuAI Performance Dashboard")
st.markdown(
    """
    <div style="background: linear-gradient(90deg, #667eea 0%, #764ba2 100%); padding: 1rem; border-radius: 10px; margin-bottom: 2rem;">
        <h3 style="color: white; margin: 0; text-align: center;">✨ Advanced Analytics & Model Performance Insights ✨</h3>
...
tab1, tab2, tab3, tab4 = st.tabs(
    ["🤖 Model Selection", "⭐ User Feedback", "📊 Complexity Analysis", "⚙️ Threshold Calibration"]
)
...
with tab4:
    st.header("⚙️ Threshold Calibration")
...
<div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 1rem; border-radius: 10px; margin: 1rem 0;">
    <p style="color: white; margin: 0; text-align: center;">
        🧙‍♂️ Help ManuAI learn and improve by providing feedback on query responses
    </p>
</div>
""", unsafe_allow_html=True)
...
# Add refresh button
if st.sidebar.button("🔄 Refresh Data", help="Reload all performance data"):
    st.experimental_rerun()

# Display timestamp
st.sidebar.markdown("---")
st.sidebar.markdown(f"🕐 **Last updated:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
st.sidebar.markdown("🧙‍♂️ *ManuAI Dashboard v1.0*")
